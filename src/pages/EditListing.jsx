...
import { addDoc, doc, collection, serverTimestamp, setDoc, getDoc, updateDoc } from 'firebase/firestore';
...
export default function CreateListing() {
      ...
      const { type, name, bedrooms, bathrooms, parking, furnished, address, description, offer, regularPrice, discountedPrice, images, imageUrls } = formData;
      ...
      const onSubmit = async (e) => {
            ...
            const imgUrls = images === undefined ? imageUrls : await Promise.all([...images].map((image) => storeImage(image)))
                  .catch((error) => {
                        setLoading(false)
      ...
      return (
            <main className='max-w-lg mx-auto px-2  mb-5'>
                  ...
                  <form onSubmit={onSubmit}>
                        ...
                        <input id='images'
                              ...
                              onChange={onchange}
                        />
                        <button type='submit' className='mt-12 mb-6 rounded w-full px-7 py-4 uppercase bg-blue-600 font-medium text-sm text-white shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out' >Update Listing</button>
                  </form>
            </main>
      )
}     
